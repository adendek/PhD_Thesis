...
One of the key parameter that describe the particle accelerator, except of beam energy, is the quantity called integrated luminosity.  The luminosity is defined as  the ratio of the  number of events detected $N$ in a certain time $t$ to the interaction cross-section $\sigma$:  
\begin{equation}
    L = \frac{1}{\sigma} \frac{dN}{dt}
\end{equation}
In practice, the integrated luminosity $\mathcal{L}_{int}$ with respect to the time is used instead. Based on this quantity one can estimate the number of expected events for a given process. Figure present the integrated luminosity delivered to each of the LHC experiments. It is clearly visible that LHCb operates at significantly lower  
...
Equation \ref{eq:eta} allows finding the relationship between the parameter $\eta$ and the angle $\vartheta$. When the angle $\theta$ is getting smaller, then the $\eta$ is rising.


The choice of such layout was motivated by the LHCb physics program, in particular the angular distribution of the $\overline{b}b$ pairs production, which are concentrated around the forward and backward cones, see figure \ref{fig:bb}.
